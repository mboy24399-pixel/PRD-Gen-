...
function clientIp(req: NextApiRequest) { const x = req.headers['x-forwarded-for']; return (typeof x === 'string' ? x.split(',')[0] : '') || req.socket.remoteAddress || 'unknown'; }
function withinRateLimit(req: NextApiRequest) {
  const key = clientIp(req); const now = Date.now(); const old = rate.get(key);
...
function originAllowed(req: NextApiRequest) {
  ...
  // BYOK requests do not use the server's secret pool, so they can be called from the GitHub Pages build.
...
function safeBaseUrl(provider: Provider, input?: string) {
  ...
  if (provider === 'custom') {
    ...
    let host = ''; try { host = new URL(value).hostname.toLowerCase(); } catch { throw new Error('Invalid custom endpoint URL.'); }
...
function requestFor(provider: Provider, model: string, prompt: string, baseUrl: string) {
  if (provider === 'gemini') return { url:`${baseUrl}/v1beta/models/${encodeURIComponent(model)}:streamGenerateContent?alt=sse`, body:{contents:[{role:'user',parts:[{text:prompt}]}],generationConfig:{temperature:.25,topP:.9,maxOutputTokens:32000}} };
  if (provider === 'anthropic') return { url:`${baseUrl}/v1/messages`, body:{model,max_tokens:32000,temperature:.25,stream:true,messages:[{role:'user',content:prompt}]} };
  return { url:`${baseUrl}/chat/completions`, body:{model,temperature:.25,top_p:.9,max_tokens:32000,stream:true,messages:[{role:'user',content:prompt}]} };
}
function textFromEvent(provider: Provider, data: string) {
  try { const j=JSON.parse(data); if(provider==='gemini')return j?.candidates?.[0]?.content?.parts?.map((p:{text?:string})=>p.text||'').join('')||''; if(provider==='anthropic')return j?.type==='content_block_delta'?j?.delta?.text||'':''; return j?.choices?.[0]?.delta?.content||''; } catch { return ''; }
}
async function streamNormalized(res: NextApiResponse, upstream: Response, provider: Provider) {
  ...
  const reader=upstream.body.getReader(); const decoder=new TextDecoder(); let buffer=''; const send=(text:string)=>{if(text)res.write(`data: ${JSON.stringify({text})}\n\n`);};
  while(true){const {value,done}=await reader.read();buffer+=decoder.decode(value||new Uint8Array(),{stream:!done});const events=buffer.split(/\n\n|\r\n\r\n/);buffer=events.pop()||'';for(const event of events){for(const line of event.split(/\r?\n/)){if(!line.startsWith('data:'))continue;const data=line.slice(5).trim();if(!data||data==='[DONE]')continue;send(textFromEvent(provider,data));}}if(done)break;}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if(!withinRateLimit(req))return json(res,429,{error:'Rate limit reached. Try again in a minute.'});
  try{
    ...
    for(const key of keys){const reqSpec=requestFor(provider,model,prompt,baseUrl);const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),90_000);try{const upstream=await fetch(reqSpec.url,{method:'POST',headers:headersFor(provider,key),body:JSON.stringify(reqSpec.body),cache:'no-store',signal:controller.signal});if(upstream.ok&&upstream.body){if(body.test){clearTimeout(timeout);return json(res,200,{ok:true,provider,model,message:'Provider authentication and request path are healthy.'});}clearTimeout(timeout);return await streamNormalized(res,upstream,provider);}lastStatus=upstream.status;lastDetail=(await upstream.text().catch(()=>'' )).slice(0,1000);if(!RETRYABLE.has(lastStatus))break;}catch(e){lastStatus=504;lastDetail=e instanceof Error?e.message:'Upstream request failed';}finally{clearTimeout(timeout);}}
